'use client'
import Link from "next/link"
import courcedata from "../data/music_cources.json"
import { BackgroundGradient } from "./ui/background-gradient"
interface cource{
        id: number,
        title: string,
        slug: string,
        description: string,
        price: number,
        instructor:string,
        isFeatured: boolean,
        // "image": "/courses/guitar.jpg" 
}
function FeaturedCources(){

    const featuredcources=courcedata.courses.filter((course:cource) =>course.isFeatured)
    
    return(
    <div className="py-12 bg-gray-900 text-white">
    <div className="text-center">
        <h1 className="text-xl text-teal-600 
           font-semibold tracking-wide uppercase
          ">FEATURED COURCES</h1>
         <p className="mt-2 text-3xl leading-8 
            font-extrabold  tracking-tight text-white"
         >Learn With The Best</p>
    </div>
         <div className="mt-10 ">
            <div className="grid grid-cols-1  sm:grid-cols-2 
               lg:grid-cols-3 gap-8 justify-center" >
               {featuredcources.map((course:cource)=>(
                  <div  key={course.id} className="flex justify-center">
                    <BackgroundGradient
                    className="flex flex-col rounded-[22px]
                     bg-white dark:bg-zinc-900
                    overflow-hidden h-full max-w-sm" >
                        <div className="p-4 sm:p-6 flex 
                        flex-col  items-center text-center flex-grow
                        ">
                            <p className="text-lg sm:text-xl text-black 
                              mt-4 mb-2 dark:text-neutral-200" 
                            >{course.title}</p>
                            <p className="text-sm  text-neutral-600 
                             dark:text-neutral-400 flex-grow" 
                            >{course.description}</p>
                            <Link href={`cources/${course.slug}`}>Learn More</Link>
                        </div>
                    </BackgroundGradient>
                  </div>
               ))}
            </div>
         </div>
           <div className="mt-20 text-center">
            <Link href={"/cources"}
             className="px-4 py-3 rounded border 
                 border-neutral-600 text-neutral-700 bg-white
                 hover:bg-gray-200 transition duration-200"
            >
                View All Cources
            </Link>
           </div>    
    </div>
    )

}
export default FeaturedCources